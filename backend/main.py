...
# # === CORS: explicitly allow Vercel domains ===
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=[
#         "https://healthcare-translation-ai.vercel.app",
#         "https://healthcare-translation-ai-git-main-sahifa-hashmis-projects.vercel.app"
#     ],
#     allow_credentials=True,
#     allow_methods=["*"],
...
#         out_path = tmp.name
#     try:
#         # Use ElevenLabs if key exists, otherwise fallback to gTTS
#         if engine == "elevenlabs" and ELEVENLABS_API_KEY:
#             text_to_speech_with_elevenlabs(text, out_path)
#         else:
#             text_to_speech_with_gtts(text, out_path)
...
# CORS: allow your Vercel frontend domains
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://healthcare-translation-ai.vercel.app",
...
# Root endpoint (works on both desktop and mobile)
@app.get("/")
def root():
    return {"status": "ok", "message": "Welcome to Healthcare Translation API"}
...
async def translate(
    ...
    try:
        # Use Groq translation (you can switch to OpenAI if needed)
        if not GROQ_API_KEY or not Groq:
            raise RuntimeError("Groq translation not available")
        client = Groq(api_key=GROQ_API_KEY)
        prompt = (
